...
namespace Horizon
{
    ...
    void Epoll::update(std::shared_ptr<Eventbase> pEvent)
    {
    ...
    void Epoll::remove(std::shared_ptr<Eventbase> pEvent)
    {
        assert(_registEvent.find(pEvent->fd()) != _registEvent.end());
    ...
    int Epoll::dispatch(int timeout,std::vector<std::shared_ptr<Eventbase>>& activelist)
    {
    ...
    void Epoll::apply_change(std::shared_ptr<Eventbase> pEvent, int op)
    {
